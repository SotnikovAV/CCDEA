...
public class SPDService extends DfService implements ISPDService {
    @Override
    public String createDocumentFromMQType(IDfSession dfSession, MCDocInfoModifySPDType documentXmlObject, String docSourceCode, String docSourceId) throws DfException {
        ...
        try {
            ...
            IDfSysObject passportSysObject = SPDPersistence.createDocument(dfSession);
            DossierKeyDetails keyDetailsFromMQ = SPDPersistence.getKeyDetailsFromMQ(documentXmlObject);
            ...
            SPDPersistence.saveFieldsToDocument(passportSysObject, documentXmlObject, dossier.getObjectId().getId(), docSourceCode, docSourceId, versionRecordDetails);
            spdObjectId = passportSysObject.getObjectId().getId();
            ...
            AuditPersistence.createActionRecord(dfSession, new DfId(spdObjectId), attachDocActionRecord);
            AuditPersistence.createVersionRecord(dfSession, new DfId(spdObjectId), versionRecordDetails);

            if (!isTransAlreadyActive) {
                dfSession.commitTrans();
            }

            DfLogger.info(this, "CreateDocumentFromMQType: {0}/{1}. Finish (new ID: {2})", new String[] {docSourceCode, docSourceId, spdObjectId}, null);
        }
    ...
    public void updateDocumentFromMQType(IDfSession dfSession, MCDocInfoModifySPDType documentXmlObject, String docSourceCode, String docSourceId, IDfId documentId) throws DfException {
        ...
        try {
            ...
            IDfSysObject passportSysObject = SPDPersistence.lockDocumentForUpdate(dfSession, documentId);
            IDfPersistentObject dossier = null;
            IDfPersistentObject oldDossier = null;
            DossierKeyDetails keyDetailsFromMQ = SPDPersistence.getKeyDetailsFromMQ(documentXmlObject);
            if (!keyDetailsFromMQ.equals(SPDPersistence.getKeyDetails(passportSysObject))) {
                dossier = DossierPersistence.getOrCreateDossierByKeyDetails(dfSession, keyDetailsFromMQ);
            }
            else {
                dossier = dfSession.getObject(SPDPersistence.getDossierId(passportSysObject));
            }
