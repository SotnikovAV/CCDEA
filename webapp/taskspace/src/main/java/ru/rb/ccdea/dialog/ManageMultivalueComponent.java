...
public class ManageMultivalueComponent extends Component {
    ...
    private String getFilteredValueListDql(String valueStart) {
        if (isContractCurrencyValue() || isDocumentCurrencyValue()) {
            ...
            return "select distinct s_doc_number as s_value, s_doc_number as s_label from ccdea_pd where s_doc_number like '" + valueStart + "%' UNION ALL " +
                    "select distinct s_doc_number as s_value, s_doc_number as s_label from ccdea_spd where s_doc_number like '" + valueStart + "%' UNION ALL " +
